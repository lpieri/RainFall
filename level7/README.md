...
Ce niveau est très similaire au niveau précédent, cette fois on peut voir que le binaire ouvre le fichier `/home/user/level8/.pass` et récupère son contenu sans l'affichée.

Une fonction `m` est présente mais pas utilisée, cette fonction affiche le contenu du fichier `.pass`.

En examinant le code on peut voir qu'il `malloc` 2 structures et copie les `argv` 1 et 2 dans le pointeur de la structure, ainsi avec le premier `strcpy` nous pouvons réecrire l'addresse du pointeur de la deuxième structure par celle de la fonction `puts` et copié à cette adresse celle de notre fonction `m`.
